...
void Dijkstra(vector<priority_queue<pair<int, int>>>& path_table, vector<bool>& visited, vector<int>& shortest_table, int start);
...
int main(void) {
	...
	cin >> V >> E;
	cin >> start;
	vector<priority_queue<pair<int, int>>>path_table(V+1);
